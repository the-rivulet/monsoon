...
import { Skill, Skillset } from "./globals/skills.js";
import { HandSlot } from "./item.js";
import { loopStart, tick } from "./loop.js";
import { Position, Solidity, walkable, world } from "./map.js";
...
class UnarmedAttackAbility extends Ability {
    ...
    n;
    addPower;
    constructor(owner, m, n, addPower = true) {
        super(owner);
        this.m = m;
        this.n = n;
        this.addPower = this.addPower;
    }
    tryActivate(targetPos) {
        if (!this.owner.actionsLeft)
            return Result.err_tired;
        if (!this.owner.pos.near(targetPos))
            return Result.err_too_far;
        if (!this.owner.pos.region.creatureAt(targetPos.x, targetPos.y))
...
export class Creature {
    ...
    team;
    constructor(x, y, r, name, team = enemyTeam) {
    ...
    destroy() {
        ...
        for (let i of this.inventory.filter(x => x.item))
            i.item.unequip(this.pos);
    }
    /** Place the creature in the world, and do other setup tasks. */
    realize() {
        if (!this.pos.region.creatures.includes(this))
            this.pos.region.creatures.push(this);
    ...
    tryStep(x, y) {
        ...
        if (this.pos.y == y)
            return ((!w1 && !w2) || (w2 && this.jumpLeft) || (!w2 && this.jumpStage != JumpStage.descent && this.jumpLeft)) ? Result.ok : Result.err_tired;
        else if (this.pos.y - 1 == y)
            return this.jumpStage == JumpStage.ascent ? Result.ok : Result.err_tired;
        else
            return Result.ok;
    }
    /**
    ...
    step(x, y) {
        ...
        if (s == Result.ok) {
            this.speedLeft -= d;
            this.refresh(false);
            for (let i of this.pos.region.items)
                i.fall();
        }
        return s;
    }
    ...
    fall() {
        let fallen = 0;
        while (this.step(this.pos.x, this.pos.y + 1) == Result.ok)
            fallen++;
        // TODO: possible agility check to reduce damage
        if (fallen > 10 && !this.flying)
            this.hp.hurt(fallen - 10);
    }
    /**
     * Pick up an item.
     ...
     */
    pickup(item, handSlot) {
        if (!this.pos.is(item.pos))
            return Result.err_too_far;
        if (handSlot && handSlot.owner != this)
            return Result.err_invalid;
        if (!this.actionsLeft)
            return Result.err_tired;
        ...
        if (!handSlot)
            return Result.err_empty; // if we still didn't find one, then we failed
     ...
     */
    throw(slot, target) {
        if (!this.pos.canSee(target))
            return Result.err_needs_sight;
        if (slot.owner != this)
            return Result.err_invalid;
        if (!slot.item)
...
class AICreature extends Creature {
}
export class Dropwig extends AICreature {
  ...
  latching onto the ceiling above for a surprise attack.`;
    ...
    render(x, y, tileSize) {
        ctx.drawImage(getId(this.id), x, y, tileSize, tileSize);
    }
    takeTurn() {
...
export class SapientCreature extends Creature {
    /** Sapient creatures have a base jump height of 4. */
    getJump = () => 4;
    adaptations = [];
    constructor(x, y, r, name, team = playerTeam) {
...
export let startRunHook = new OneHookable((char) => {
    if (tick > 0)
        return;
 ...
 */
export function registerCharacter(char) {
