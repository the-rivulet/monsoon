...
import { displayInfo } from "./globals/mod.js";
let loadedMods = [];
let appliedMods = [];
function unapplyAll() {
...
getId("fileloader").onclick = async function () {
    ...
    let loadedMod = { info: undefined, files: [] };
    let modInfoReader = new FileReader();
    let modInfoCallback = () => {
        try {
            let r = JSON.parse(modInfoReader.result.toString());
            if (!r) {
    ...
    for await (let item of directory.values()) {
        if (item.kind == "file") {
            ...
            if (file.name == "modinfo.json" && file.type == "application/json") {
                if (file)
                    modInfoReader.readAsText(file);
                else
                    alert("ERROR::LOADFILES::'failed to load modinfo.json'");
            }
                ...
                if (file) {
                    ...
                    alert("ERROR::LOADFILES::'failed to load main.js'");
            }
            else {
                ...
                if (file) {
                    ...
                    alert("ERROR::LOADFILES::'failed to load " + item.name + "'");
            }
        }
    }
};
...
export let applyModHook = new OneHookable((pack) => {
    ...
    try {
        ...
        for (let index = 0; index < loadedMods.length; index++) {
            ...
            for (let dependency of mod.info.dependencies) {
                ...
                if (!foundMod)
                    alert("ERROR::APPLYMODS::'" + mod.info.name + "';'dependency missing or misplaced';" + dependency.id + "';'v" + dependency.version + "'");
                else if (!satisfies(dependency.version, new SlugverRange(foundMod.info.modVersion)))
                    alert("ERROR::APPLYMODS::'" + mod.info.name + "';'required version';'" +
            ...
            if (mod.script) {
                ...
                alert("NOTE::'" + mod.info.name + "'::'no script'");
        }
